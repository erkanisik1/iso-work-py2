...
def setup_live_policykit_conf(project):
    ...
    policykit_pisi_conf_tmpl2 = """polkit.addRule(function(action, subject) {
    ...
    policykit_conf_tmpl3 = """polkit.addRule(function(action, subject) {
    ...
    destPisi = os.path.join(image_dir, "usr/share/polkit-1/rules.d/10-nopasswd.rules")
...
def squash_live_config_image(project):
    ...
    if project.type == "live":
        ...
        if 'plasma-workspace' in project.all_install_image_packages:
            ...
            os.system("cp -rf ./data/etc/skel/ {}/etc/".format(config_image_dir))
...
def squash_image(project):
    ...
    # yapıldıktan sonraki yapılacak işlemler için  bu bölüm kullanılabilir
    shutil.copy("./data/etc/pisilinux-release", "{}/etc/".format(image_dir))
    shutil.copy("./data/etc/lsb-release", "{}/etc/".format(image_dir))
    shutil.copy("./data/etc/os-release", "{}/etc/".format(image_dir))
    shutil.copy("./data/etc/hostname", "{}/etc/".format(image_dir))
        ...
        if project.type != "install":
            obj.addUser(1000, "pisi", "Pisi Linux", "/home/pisi", "/bin/bash",
                        "live", ["wheel", "root", "users", "lp", "lpadmin", "cdrom",
                                 ...
                                 "dialout"], [], [],
                        dbus_interface="tr.org.pardus.comar.User.Manager")
            # Live kullanıcısına şifresiz sudo erişimi ekle
            
            polkit_rules_path = os.path.join(image_dir, "etc/polkit-1/rules.d/10-pisi-nopasswd.rules")

            with open(polkit_rules_path, "w") as f:
                f.write('polkit.addRule(function(action, subject) {\n')
                f.write('    if (subject.user == "pisi") {\n')
                f.write('        return polkit.Result.YES;\n')
                f.write('    }\n')
                f.write('});\n')

            os.chmod(polkit_rules_path, 0o644)
...
def make_EFI(project, grub=True):
    ...
    if grub:
        ...
        run("cp -rf %s/grub.cfg %s/EFI/BOOT/grub.cfg" % (image_dir, iso_dir),
            ignore_error=True)
    ...
    if grub:
        ...
        #   ignore_error=True)
        run("cp -rf %s/grubx64.efi %s/EFI/BOOT/BOOTX64.EFI" % (image_dir, iso_dir),
            ignore_error=True)
        run("cp -rf %s/grubx64.efi %s/EFI/BOOT/BOOTX64.EFI" % (image_dir, efi_tmp),
            ignore_error=True)
        run("cp -rf %s/grubx64.efi %s/EFI/BOOT/GRUBX64.EFI" % (image_dir, iso_dir),
           ignore_error=True)
        run("cp -rf %s/grubx64.efi %s/EFI/BOOT/GRUBX64.EFI" % (image_dir, efi_tmp),
            ignore_error=True)
...
def make_iso(project, toUSB=False, dev="/dev/sdc1"):
    ...
    try:
        ...
        -publisher "%s" \
        -A "%s" \
        %s' % (label, iso_file, iso_dir, publisher, application, iso_dir)
